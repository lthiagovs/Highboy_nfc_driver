/**
 * @file main.c
 * @brief High Boy NFC — Flipper Zero-style MIFARE Classic Reader v3.
 *
 ...
 *   - PRNG nonce analysis (static nonce → clone detection)
...
static mfc_emu_card_data_t s_emu_card = { 0 };
static bool                s_emu_data_ready = false;
...
static const char* get_manufacturer_name(uint8_t uid0)
{
    switch (uid0) {
...
static card_type_info_t identify_card(uint8_t sak, const uint8_t atqa[2])
{
...
static void mf_classic_read_full(nfc_iso14443a_data_t* card)
{
...
static void mful_read_all_ui(nfc_iso14443a_data_t* card)
{
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
...
void app_main(void)
{
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
    ESP_LOGI(TAG, "║  HIGH BOY NFC — ST25R3916 Reader v3              ║");
    ESP_LOGI(TAG, "║  Crypto1 + Flipper Zero-style Analysis           ║");
    ESP_LOGI(TAG, "╚══════════════════════════════════════════════════╝");
    ESP_LOGI(TAG, "");
    ...
    ESP_LOGI(TAG, "║  ATQA:   %s", atqa_str);
    ESP_LOGI(TAG, "║  SAK:    0x%02X (0b%d%d%d%d%d%d%d%d)", card.sak,
    ...
    if (mfr) {
        ESP_LOGI(TAG, "║  Chip:   %s", mfr);
    }

    /* Protocol info */
    if (type_info.is_mf_classic) {
        ESP_LOGI(TAG, "║  Proto:  ISO 14443-3A + Crypto1 (proprietário NXP)");
    } else if (type_info.is_mf_ultralight) {
        ESP_LOGI(TAG, "║  Proto:  ISO 14443-3A + Ultralight (3-pass auth)");
    } else if (type_info.is_iso_dep) {
        ESP_LOGI(TAG, "║  Proto:  ISO 14443-4 (T=CL / ISO-DEP)");
    }
    ...
    if (s_emu_data_ready && type_info.is_mf_classic) {
        ESP_LOGI(TAG, "");
        ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
        ESP_LOGI(TAG, "║  🔄 MODO EMULAÇÃO                                ║");
        ESP_LOGI(TAG, "║                                                  ║");
        ESP_LOGI(TAG, "║  Dump completo obtido!                           ║");
        ESP_LOGI(TAG, "║                                                  ║");
        ESP_LOGI(TAG, "║  ⚡ RETIRE O CARTÃO AGORA!                       ║");
        ESP_LOGI(TAG, "║                                                  ║");
        ESP_LOGI(TAG, "║  Aguardando 5 segundos...                        ║");
        ESP_LOGI(TAG, "║  Depois, aproxime um LEITOR (celular/Flipper).   ║");
        ESP_LOGI(TAG, "║  Emulação rodará por 60 segundos.                ║");
        ESP_LOGI(TAG, "╚══════════════════════════════════════════════════╝");
        ESP_LOGI(TAG, "");

        /* Wait for user to remove card */
        for (int i = 5; i > 0; i--) {
            ESP_LOGI(TAG, "  Iniciando emulação em %d...", i);
        ...
        ESP_LOGI(TAG, "");
        ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
        ESP_LOGI(TAG, "║  📡 EMULANDO MIFARE Classic 1K                   ║");
        ESP_LOGI(TAG, "║                                                  ║");
        ESP_LOGI(TAG, "║  UID: %02X %02X %02X %02X                              ║",
                 s_emu_card.uid[0], s_emu_card.uid[1],
                 s_emu_card.uid[2], s_emu_card.uid[3]);
        ESP_LOGI(TAG, "║  SAK: 0x%02X  ATQA: %02X %02X                        ║",
                 s_emu_card.sak, s_emu_card.atqa[0], s_emu_card.atqa[1]);
        ESP_LOGI(TAG, "║                                                  ║");
        ESP_LOGI(TAG, "║  Aproxime um leitor NFC para testar...           ║");
        ESP_LOGI(TAG, "╚══════════════════════════════════════════════════╝");
        ESP_LOGI(TAG, "");

        /* Run emulation loop for 60 seconds */
        ...
        while ((esp_timer_get_time() - start_time) < timeout_us) {
            ...
            /* Log state changes */
            if (state != last_state) {
                ESP_LOGI(TAG, "  Estado: %s → %s",
                         mfc_emu_state_str(last_state),
                         mfc_emu_state_str(state));
                last_state = state;
            }

            /* Yield to FreeRTOS to feed watchdog */
        ...
        /* Show statistics */
        mfc_emu_stats_t stats = mfc_emu_get_stats();

        ESP_LOGI(TAG, "");
        ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
        ESP_LOGI(TAG, "║  📊 ESTATÍSTICAS DA EMULAÇÃO                     ║");
        ESP_LOGI(TAG, "╠══════════════════════════════════════════════════╣");
        ESP_LOGI(TAG, "║  Ativações (selecionado):  %3d                   ║", stats.cycles);
        ESP_LOGI(TAG, "║  Auth tentativas:          %3d                   ║", stats.total_auths);
        ESP_LOGI(TAG, "║  Auth sucesso:             %3d                   ║", stats.successful_auths);
        ESP_LOGI(TAG, "║  Auth falha:               %3d                   ║", stats.failed_auths);
        ESP_LOGI(TAG, "║  Reads servidos:           %3d                   ║", stats.reads_served);
        ESP_LOGI(TAG, "║  Writes servidos:          %3d                   ║", stats.writes_served);
        ESP_LOGI(TAG, "║  HALTs recebidos:          %3d                   ║", stats.halts);
        ESP_LOGI(TAG, "║  Comandos desconhecidos:   %3d                   ║", stats.unknown_cmds);
        ESP_LOGI(TAG, "╚══════════════════════════════════════════════════╝");
    }
    else {
        ESP_LOGI(TAG, "");
        ESP_LOGI(TAG, "╔══════════════════════════════════════════════════╗");
        ESP_LOGI(TAG, "║  ✅ Leitura concluída.                            ║");
        if (!s_emu_data_ready) {
            ESP_LOGW(TAG, "║  ⚠ Dump incompleto — emulação não disponível.   ║");
        }
        ESP_LOGI(TAG, "╚══════════════════════════════════════════════════╝");
    }
